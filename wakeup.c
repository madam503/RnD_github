...
// Audio mute/unmute flags and timer
static bool is_unmute_timer_running = false;
static time_t unmute_start_time;

static void mute_audio(void) {
    system("amixer set Master mute > /dev/null 2>&1");
}

static void unmute_audio(void) {
...
int picovoice_main(int argc, char *argv[]) {
    ...
    pv_status_t porcupine_status;
    pv_porcupine_t *porcupine = NULL;
    porcupine_status = pv_porcupine_init_func(access_key, model_path, 1, &keyword_path, &sensitivity, &porcupine);
    ...
    while (!is_interrupted) {
        ...
        if (keyword_index != -1) {
            ...
            fflush(stdout);
            // Unmute and set volume for 5 seconds
            unmute_audio();
            is_unmute_timer_running = true;
            unmute_start_time = time(NULL);
        }

        // After 5 seconds, mute again
        if (is_unmute_timer_running && difftime(time(NULL), unmute_start_time) >= 5.0) {
            mute_audio();
            is_unmute_timer_running = false;
        }
    }
...
#define NULL_TERMINATED       (-1)
    ...
    return picovoice_main(argc, argv);
}
